...
def embdi_table2graph(indir, outdir, cfg):
    """
    ...
    with open(outdir / "edgelist", "w") as edgelist:
        for _, edge in edge_df.iterrows():
            val, row, col, weight = edge["val"], edge["row"], edge["col"], edge["weight"]
...
def make_edge_df(df):
    """
    Stack df so each row corresponds to one value in the original df, with columns row, col, val.
...
def format_rows_cols(edge_df):
    """
    Remove 'table' column but incorporate into row and col tokens
    """
    edge_df["row"] = edge_df["table"] + "_row:" + edge_df["row"]
    edge_df["col"] = edge_df["table"] + "_col:" + edge_df["col"]
    return edge_df[["row", "col", "val"]]
